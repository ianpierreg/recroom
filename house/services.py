...
class CosineCalculator:
    ...
    def calculate_similarity_house_tenant(self, house, future_tenant):
        ...
        for interest_type in interest_types:
            interests_future_tenant_boolean += self.get_boolean_list(interests_future_tenant.get(interest_type.name), interest_type.name)
            for i, tenant_interests in enumerate(tenants_interests):
                tenants_interests_boolean[i] += self.get_boolean_list(tenant_interests.get(interest_type.name), interest_type.name)

        similarity = 0

        for tenant_interests_boolean in tenants_interests_boolean:
            similarity += self.get_similarity(interests_future_tenant_boolean, tenant_interests_boolean)
